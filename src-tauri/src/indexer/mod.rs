...
mod tests {
    ...
    // --- is_graph_source_file: comprehensive language coverage ---

    #[test]
    fn graph_source_file_accepts_all_parser_languages() {
        let supported = [
            "main.js", "index.mjs", "lib.cjs", "app.jsx",
            "main.ts", "index.mts", "lib.cts", "component.tsx",
            "main.rs", "server.go", "script.py",
            "App.java", "Main.kt", "View.swift", "controller.rb", "index.php",
            "util.c", "header.h", "engine.cpp", "types.hpp", "Service.cs",
            "build.sh", "run.bash", "init.zsh", "setup.fish",
            "schema.sql", "config.json", "settings.jsonc",
            "docker-compose.yaml", "values.yml",
            "Cargo.toml", "settings.ini", "app.cfg", "server.conf",
            "pom.xml", "style.css", "theme.scss", "layout.less",
            "App.vue", "Counter.svelte", "Page.astro",
        ];
        for file in supported {
            assert!(
                is_graph_source_file(Path::new(file)),
                "{file} should be a graph source file"
            );
        }
    }

    #[test]
    fn graph_source_file_rejects_non_parseable_files() {
        let unsupported = [
            "image.png", "archive.zip", "data.pdf", "binary.exe",
            "README.md", "notes.txt", "Dockerfile",
        ];
        for file in unsupported {
            assert!(
                !is_graph_source_file(Path::new(file)),
                "{file} should NOT be a graph source file"
            );
        }
    }

    // --- is_indexable_file: extension coverage ---

    #[test]
    fn indexable_file_accepts_all_documented_extensions() {
        let extensions = [
            "main.js", "index.mjs", "lib.cjs", "app.jsx",
            "main.ts", "index.mts", "lib.cts", "component.tsx",
            "main.rs", "server.go", "script.py",
            "App.java", "Main.kt", "Main.kts", "View.swift",
            "controller.rb", "index.php",
            "util.c", "header.h", "engine.cpp", "types.hpp", "Service.cs",
            "build.sh", "run.bash", "init.zsh", "setup.fish",
            "schema.sql", "config.json", "settings.jsonc",
            "docker-compose.yaml", "values.yml",
        ...
        for file in extensions {
            assert!(
                is_indexable_file(Path::new(file)),
    ...
    #[test]
    fn indexable_file_rejects_binary_and_media_extensions() {
        let unsupported = [
            "image.png", "photo.jpg", "archive.zip", "binary.exe",
            "data.pdf", "library.so", "object.o", "key.pem",
            "cert.p12",
        ];
        for file in unsupported {
            assert!(
                !is_indexable_file(Path::new(file)),
                "{file} should NOT be indexable"
            );
        }
    }

    // --- is_extensionless_indexable_file: named files ---

    #[test]
    fn extensionless_indexable_accepts_named_files() {
        use super::is_extensionless_indexable_file;

        let named_files = [
            "Dockerfile", "dockerfile", "Containerfile",
            "Makefile", "makefile", "justfile",
            "Procfile", "Brewfile", "Vagrantfile",
            "Jenkinsfile", "Tiltfile", "README", "LICENSE",
        ];
        for name in named_files {
            assert!(
                is_extensionless_indexable_file(Path::new(name)),
                "{name} should be extensionless indexable"
            );
        }
    }

    #[test]
    fn extensionless_indexable_rejects_unknown_names() {
        use super::is_extensionless_indexable_file;

        let unknown = ["NOTES", "TEMP", "random", "build_output"];
        for name in unknown {
            assert!(
                !is_extensionless_indexable_file(Path::new(name)),
                "{name} should NOT be extensionless indexable"
            );
        }
    }

    #[test]
    fn extensionless_indexable_rejects_files_with_extensions() {
        use super::is_extensionless_indexable_file;

        // Even if named "Dockerfile.bak", it has an extension so should be rejected
        assert!(!is_extensionless_indexable_file(Path::new("Dockerfile.bak")));
    }

    // --- is_ignored_path: comprehensive coverage ---

    #[test]
    fn ignored_path_catches_all_ignored_directories() {
        let ignored = [
            "node_modules/lodash/index.js",
            ".git/config",
            ".localbrain/metadata.db",
            "vendor/pkg/lib.go",
            ".venv/lib/python.py",
    ...
    #[test]
    fn ignored_path_does_not_reject_regular_source_files() {
        let allowed = [
            "src/main.rs",
            "lib/utils.ts",
            "app/models/user.rb",
    ...
    #[test]
    fn ignored_path_catches_sensitive_file_extensions() {
        let sensitive = [
            "config/server.key",
            "certs/ca.pem",
            "store/keystore.p12",
        ...
        for path in sensitive {
            assert!(
                is_ignored_path(Path::new(path)),
                "{path} should be ignored as sensitive"
            );
        }
    }
}
